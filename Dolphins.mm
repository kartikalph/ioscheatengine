...
kaddr BulletTrack;
bool 加速读取;
static bool 是否有效取敌 = false;
static ImVec3 AimMuBiao;
static NSTimer *偏移遍历;
float D2D;

static bool isHookAngle = false;
bool 静默 = true;
static float tDistance = 0, tDis = 0, markDistance, markDis;
static uintptr_t tmpGunBase = 0;//手持

static std::string playerDataKillweaponName = "拳头";//自瞄对象手持武器

static int playerDataKillHP = 0;//自瞄对象血量

static std::string playerDataKillName = "NULL";//自瞄对象名字

static int playerDataKillActor = 0;//自瞄对象杀敌人数

static int playerDataweaponAmmo = 0;//手持武器子弹数量

static int playerDataweaponAmmoMax = 0;//手持武器最大子弹数量

static uintptr_t myGunBase = 0;


static int skinX = 3;//衣服
static int skinakm = 4;//AKM淘汰
static int skinm4 = 1;//M4淘汰
typedef long shootWeaponEntityComp;
...
static long gWorld(){
    ...
    if ([BundID containsString:@"ig"]) {
        gWorldaddr = 0x1029D1558;
        gWorldData = 0x10A8BB0E0;
    } if ([BundID containsString:@"kr"]) {
        gWorldaddr = 0x102BE44DC;
        gWorldData = 0x10AC72D90;
    }if ([BundID containsString:@"rekoo"]) {
        gWorldaddr = 0x102BBB7D8;
        gWorldData = 0x10AC4CC90;
    }if ([BundID containsString:@"vn"]) {
        gWorldaddr = 0x1029877D8;
        gWorldData = 0x10A967410;
    }if ([BundID containsString:@"imo"]) {
        gWorldaddr = 0x1024CB964;
        gWorldData = 0x109ED2960;
    }
...
static long gName(){
    ...
    NSString *BundID = [infoDictionary objectForKey:@"CFBundleIdentifier"];
    if ([BundID containsString:@"ig"]) {
        gNameaddr = 0x1049A3510;
        gNameData = 0x10A217E50;
    } if ([BundID containsString:@"kr"]) {
        gNameaddr = 0x104C630A0;
        gNameData = 0x10A5CE930;
    }if ([BundID containsString:@"rekoo"]) {
        gNameaddr = 0x104C3A12C;
        gNameData = 0x10A5A8830;
    }if ([BundID containsString:@"vn"]) {
        gNameaddr = 0x104A0612C;
        gNameData = 0x10A2C32A0;
    }if ([BundID containsString:@"imo"]) {
        gNameaddr = 0x1043A93A4;
        gNameData = 0x1098368F0;
}
...
__attribute__((constructor)) static void initialize() {
   ...
   //加载hook
    ...
    //追踪线程
//    pthread_t silenceBulletTrackThread;
    //pthread_create(&silenceBulletTrackThread, nullptr, silenceBulletTrack, nullptr);
   
}
Box3D GetBox3D(MinimalViewInfo pov,ImVec3 origin, ImVec3 extends, ImVec2 screenPos) {
    ...
    ImVec3 one = origin;
    ImVec3 two = origin; two.x += extends.x;
    ImVec3 tree = origin; tree.x += extends.x; tree.y += extends.y;
    ImVec3 four = origin; four.y += extends.y;
    
    ImVec3 five = one; five.z += extends.z;
    ImVec3 six = two; six.z += extends.z;
    ImVec3 seven = tree; seven.z += extends.z;
    ImVec3 eight = four; eight.z += extends.z;
    
    ImVec2 s1, s2, s3, s4, s5, s6, s7, s8;
    
    s1 = worldToScreen(one, pov,screenPos);
    s2 = worldToScreen(two, pov,screenPos);
    s3 = worldToScreen(tree, pov,screenPos);
        ...
        if(moduleControl.systemStatus != TransmissionNormal){
            ...
            for (int index = 0; index < objectCount; ++index) {
                ...
                if (strstr(className.c_str(), "PlayerPawn") || (strstr(className.c_str(), "PlayerCharacter") || (strstr(className.c_str(), "PlayerControllertSl") || (strstr(className.c_str(), "_PlayerPawn_TPlanAI_C")|| (strstr(className.c_str(), "CharacterModelTaget")|| (strstr(className.c_str(), "FakePlayer_AIPawn")!= 0 && moduleControl.mainSwitch.playerStatus)) )))) {
                    ...
                    bool 人机 = memoryTools.readInt(objectAddr + PubgOffset::ObjectParam::RobotOffset);
                    if(moduleControl.playerSwitch.过滤人机 && 人机){
                                        continue;
                                    }
                    //队伍ID
    ...
    if (moduleControl.systemStatus == TransmissionNormal) {
        ...
        if (moduleControl.mainSwitch.playerStatus) {
            for (auto staticPlayerData: staticData.playerDataList) {
                if (memoryTools.readBool(staticPlayerData.addr + PubgOffset::ObjectParam::DeadOffset)){
                    continue;
                }
                //死亡
                //坐标
...
//人物
                ...
                BoxSize.y = 75.f;
                BoxSize.z = 163.f;
                Box3D ObjBox3D = GetBox3D(pov,objectCoord, BoxSize, screenSize);
                playerData.objbox3d.s1 = ObjBox3D.s1;
                playerData.objbox3d.s2 = ObjBox3D.s2;
                playerData.objbox3d.s3 = ObjBox3D.s3;
                playerData.objbox3d.s4 = ObjBox3D.s4;
                playerData.objbox3d.s5 = ObjBox3D.s5;
                playerData.objbox3d.s6 = ObjBox3D.s6;
                playerData.objbox3d.s7 = ObjBox3D.s7;
                playerData.objbox3d.s8 = ObjBox3D.s8;
                ...
                uintptr_t Hea = memoryTools.readFloat(staticPlayerData.addr + PubgOffset::ObjectParam::HpOffset);
                uintptr_t HeaMax = memoryTools.readFloat(staticPlayerData.addr + PubgOffset::ObjectParam::HpmaxOffset);
                
                //血量
                playerData.hp = 100 * Hea / HeaMax;
                //取敌人动作
             //   NSLog(@"****： %id",statusName);
                ...
                playerData.KillAmmo = playerDataweaponAmmo;
                playerData.KillAmmoMax = playerDataweaponAmmoMax;
                /*ENG*/
                myGunBase = memoryTools.readPtr(memoryTools.readPtr(staticData.selfAddr + PubgOffset::ObjectParam::WeaponOneOffset) + PubgOffset::ObjectParam::WeaponParam::WeaponAttrOffset);
                ...
                //传输人物坐标到绘图
                playerData.AimCoord = AimMuBiao;
                playerData.AimType = 是否有效取敌;
                
                playerData.AimScreen = worldToScreen(AimMuBiao, pov, screenSize);//X
                ...
                //屏幕XY
                playerData.screen1 = worldToScreen(objectCoord, pov, screenSize);//X
                //宽度和高度
                ImVec2 width1 = worldToScreen(ImVec3(objectCoord.x,objectCoord.y,objectCoord.z + 100), pov,screenSize);
                ImVec2 height1 = worldToScreen(ImVec3(objectCoord.x,objectCoord.y,objectCoord.z + objectHeight), pov,screenSize);
                playerData.size1.x = (playerData.screen1.y - width1.y) / 2;
                playerData.size1.y = playerData.screen1.y - height1.y;
                
                uintptr_t meshAddr = memoryTools.readPtr(staticPlayerData.addr + PubgOffset::ObjectParam::MeshOffset);
                int boneCount = memoryTools.readInt(meshAddr + PubgOffset::ObjectParam::boneCountOffset);
                ...
                if (boneCount==67){
                    ...
                    if (getBone2d(pov,screenSize, humanAddr, boneAddr, 4, bonesData.pit))//脖子
                    if (getBone2d(pov,screenSize, humanAddr, boneAddr, 3, bonesData.tripe))//胸部
                    if (getBone2d(pov,screenSize, humanAddr, boneAddr, 2, bonesData.abdomen))//上腹
                    if (getBone2d(pov,screenSize, humanAddr, boneAddr, 1, bonesData.pelvis))//肚子
                    if (getBone2d(pov,screenSize, humanAddr, boneAddr, 0, bonesData.jb))//jb

//                    if (getBone2d(pov,screenSize, humanAddr, boneAddr, 12, bonesData.lShoulder))//左甲骨
//                    if   (getBone2d(pov, screenSize,humanAddr, boneAddr, 33, bonesData.rShoulder))//右甲骨
                    if (getBone2d(pov,screenSize, humanAddr, boneAddr, 13, bonesData.lcollar))//左肩
                    if   (getBone2d(pov, screenSize,humanAddr, boneAddr, 34, bonesData.rcollar))//右肩
                    if (getBone2d(pov,screenSize, humanAddr, boneAddr, 14, bonesData.lelbow))//左手肘
                    if (getBone2d(pov,screenSize, humanAddr, boneAddr, 35, bonesData.relbow))//右手肘
                    if (getBone2d(pov,screenSize, humanAddr, boneAddr, 15, bonesData.lwrist))//左手腕                                                                          
                    if (getBone2d(pov,screenSize, humanAddr, boneAddr, 36, bonesData.rwrist))//右手腕

//腿部
                                                    if (getBone2d(pov,screenSize,humanAddr, boneAddr, 54, bonesData.lthigh))//左大腿
                                                            if (getBone2d(pov,screenSize,humanAddr, boneAddr, 55, bonesData.lknee))//左膝盖
                                                                    if (getBone2d(pov,screenSize,humanAddr, boneAddr, 56, bonesData.lankle))//左脚腕
                                                        
if (getBone2d(pov,screenSize,humanAddr, boneAddr, 58, bonesData.rthigh))//右大腿
                                                                if (getBone2d(pov,screenSize,humanAddr, boneAddr, 59, bonesData.rknee))//右膝盖
                                                                        if (getBone2d(pov,screenSize,humanAddr, boneAddr, 60, bonesData.rankle))//右脚腕
...
//腰杆
                    ...
                    if (getBone2d(pov,screenSize, humanAddr, boneAddr, 1, bonesData.pelvis))//肚子
                    if (getBone2d(pov,screenSize, humanAddr, boneAddr, 0, bonesData.jb))//jb
//                    if (getBone2d(pov,screenSize, humanAddr, boneAddr, 11, bonesData.lShoulder))//左甲骨
//                    if   (getBone2d(pov, screenSize,humanAddr, boneAddr, 32, bonesData.rShoulder))//右甲骨
                                ...
                                if (getBone2d(pov, screenSize,humanAddr, boneAddr, 32, bonesData.rcollar))//右肩
                                    if (getBone2d(pov,screenSize, humanAddr, boneAddr, 12, bonesData.lelbow))//左手肘
                                        if (getBone2d(pov,screenSize, humanAddr, boneAddr, 33, bonesData.relbow))//右手肘
                                            if (getBone2d(pov,screenSize, humanAddr, boneAddr, 13, bonesData.lwrist))//左手腕
                                                if (getBone2d(pov,screenSize, humanAddr, boneAddr, 34, bonesData.rwrist))//右手腕
...
//载具
                ...
                BoxSize.z = 300.f;
                Box3D ObjBox3D = GetBox3D(pov,objectCoord, BoxSize, screenSize);
        materialData.objbox3d.s1 = ObjBox3D.s1;
        materialData.objbox3d.s2 = ObjBox3D.s2;
        materialData.objbox3d.s3 = ObjBox3D.s3;
        materialData.objbox3d.s4 = ObjBox3D.s4;
        materialData.objbox3d.s5 = ObjBox3D.s5;
        materialData.objbox3d.s6 = ObjBox3D.s6;
        materialData.objbox3d.s7 = ObjBox3D.s7;
        materialData.objbox3d.s8 = ObjBox3D.s8;
                //角度
                materialData.angle = lateralAngleView - rotateAngle(selfCoord, objectCoord) - 180;
                //载具组件
                uintptr_t VehicleCommonComponent = memoryTools.readPtr(staticMaterialData.addr + PubgOffset::ObjectParam::VehicleCommonComponentOffset);
                //载具血量
                uintptr_t VehicleHP = memoryTools.readFloat(VehicleCommonComponent + PubgOffset::ObjectParam::VehicleHPOffset);
                //载具油量
                uintptr_t VehicleFuel = memoryTools.readFloat(VehicleCommonComponent + PubgOffset::ObjectParam::VehicleFuelOffset);
                //载具最大血量
                uintptr_t VehicleHPMax = memoryTools.readFloat(VehicleCommonComponent + PubgOffset::ObjectParam::VehicleHPMaxOffset);
                //载具最大油量
                uintptr_t VehicleFuelMax = memoryTools.readFloat(VehicleCommonComponent + PubgOffset::ObjectParam::VehicleFuelMaxOffset);
                materialData.VehicleHP = 100 * VehicleHP / VehicleHPMax;
                materialData.VehicleFuel = 100 * VehicleFuel / VehicleFuelMax;
...
AimMuBiao = aimbotCoord;                                        //跳出循环
                ...
                是否有效取敌 = true;
                  ...
                  //bool 忽略人机 = memoryTools.readBool(aimbotPlayerData.addr + PubgOffset::ObjectParam::RobotOffset);
                  //bool 人机 = memoryTools.readInt(objectAddr + PubgOffset::ObjectParam::RobotOffset);
                  //if(moduleControl.aimbotController.忽略人机){
                        //aimbotCoord = {0, 0, 0};
                        //continue;
                    //}
